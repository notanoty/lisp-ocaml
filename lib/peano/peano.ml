...
let rec exp_to_string = function
  ...
  | S_expr (x, S_expr (y, z)) ->
      "( " ^ exp_to_string x ^ exp_to_string_no_par (S_expr (y, z)) ^ ") "
  | S_expr (x, Nil) -> "( " ^ exp_to_string x ^ ") "
  | S_expr (x, y) ->
      "( " ^ exp_to_string x ^ ". " ^ exp_to_string_no_par y ^ ") "
  | Closure _ -> "(Closure)"
...
let rec list_string_to_exp list =
  match list with
  | [] -> Nil
  | head :: [] -> S_expr (String head, Nil)
  | head :: tail -> S_expr (String head, list_string_to_exp tail)
